...
const outCsv = path.join(__dirname, 'proposed-mappings-unmapped-caps-heuristic.csv');
...
function chooseProposals(reqText) {
  ...
  const proposals = [];
  // rules: audit/evidence -> SOX, discovery/onboard -> DIR-3.1, provisioning -> DIR-2.1, logging -> DIR-6.1, abac/eligib -> NIST AC-2
  ...
  if (txt.includes('discovery') || txt.includes('onboard') || txt.includes('onboarding')) {
    ...
    proposals.push({ frameworkId: 'SOX-FRAMEWORK', frameworkName: 'SOX Controls', controlId: 'SOX-ITGC-01', controlTitle: 'SOX-ITGC-01 (auto)', reason: 'keyword-discovery', score: 2 });
  }
  if (txt.includes('provision') || txt.includes('provisioning') || txt.includes('group provision')) {
  ...
  if (proposals.length === 0) {
    ...
    proposals.push({ frameworkId: 'SOX-FRAMEWORK', frameworkName: 'SOX Controls', controlId: 'SOX-ITGC-01', controlTitle: 'SOX-ITGC-01 (auto)', reason: 'fallback', score: 1 });
  }
...
function main() {
  ...
  capIds.forEach((rid) => {
    ...
    if (!req) return;
    const cand = chooseProposals(req.req || req.requirement || req.description || '');
    cand.forEach(c => proposals.push({ requirementId: rid, requirementText: req.req || '', proposalFrameworkId: c.frameworkId, proposalFrameworkName: c.frameworkName, proposalControlId: c.controlId, proposalControlTitle: c.controlTitle, reason: c.reason, score: c.score }));
  });

  writeCsv(proposals);
  fs.writeFileSync(path.join(__dirname, 'proposed-mappings-unmapped-caps-heuristic.json'), JSON.stringify({ generatedAt: new Date().toISOString(), count: proposals.length, proposals }, null, 2));
